...
Outputs by default:
- generated_mk64_php/players/index.php
- generated_mk64_php/players/<player>.php
...
SITE_ROOT_REL = "../"  # from /mk64/players/*.php to /mk64/
...
def php_header(title: str, description: str = "") -> str:
    # Safe fallback layout. If your MK64 site has shared include files, this will use them.
    title_e = e(title)
...
def render_recent_vs(profile: dict[str, Any]) -> str:
    ...
    for m in matches:
        ...
        rows.append(f"<tr><td>{e(m.get('date', 'unknown'))}</td><td>{score_text}{source}</td></tr>")
...
def render_recent_gp(profile: dict[str, Any]) -> str:
    ...
    for m in matches:
        ...
        url = m.get("jump_url", "")
        source = f' <a href="{e(url)}">source</a>' if url else ""
        rows.append(f"<tr><td>{e(m.get('date', 'unknown'))}</td><td>{winner}</td><td>{score_text}{source}</td></tr>")
    return f"""<table class=\"data-table\">
...
def render_player_page(profile: dict[str, Any]) -> str:
    ...
    description = f"MK64 Switch continuity profile for {name}, including VS, GP, match history, roles, and community notes."

    vs = profile.get("vs", {})
    gp = profile.get("gp", {})

    body = f"""{php_header(title, description)}
<section class=\"panel\">
...
def render_index(index: dict[str, Any]) -> str:
    ...
    rows = []
    for p in sorted(players, key=lambda x: (-int(x.get("total_recorded_matches", 0)), str(x.get("display_name", "")).lower())):
    ...
    return f"""{php_header('MK64 Switch Player Profiles', 'MK64 Switch player profiles generated from VS, GP, and continuity records.')}
<section class=\"panel\">
...
<section class=\"panel\">
  ...
  <table class=\"data-table\">
    <thead><tr><th>Player</th><th>First Seen</th><th>Latest Record</th><th>VS Matches</th><th>GP Matches</th><th>Total</th></tr></thead>
    <tbody>{''.join(rows)}</tbody>
  </table>
</section>
{php_footer()}"""
...
def generate(input_dir: Path, output_dir: Path) -> None:
    ...
    if not index:
        raise FileNotFoundError(f"Missing {input_dir / 'index.json'}. Run python export_continuity.py first.")
    ...
    for player in index.get("players", []):
        ...
        if not profile:
            continue
        (output_dir / f"{pid}.php").write_text(render_player_page(profile), encoding="utf-8")
        count += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate MK64 player PHP pages from continuity exports.")
    parser.add_argument("--input", default=str(INPUT_DIR), help="Continuity players export directory")
